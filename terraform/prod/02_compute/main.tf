# terraform/prod/02_compute/main.tf
...
terraform {
  ...
  backend "s3" {
    ...
    key            = "env:/prod/prod/compute.tfstate"     # Unique key for production state
...
locals {
  ...
  glue_database_name = "${var.project_name}_prod" # e.g., insightflow_prod
    ...
    Statement = [
      # S3 Access
      {
        ...
        Resource = [
          ...
          "arn:aws:s3:::insightflow-prod-processed-data/*",
          "arn:aws:s3:::insightflow-prod-raw-data",
          "arn:aws:s3:::insightflow-prod-raw-data/*"
        ]
      },
