...
data "terraform_remote_state" "storage" {
  ...
  config = {
    ...
    key    = "env:/prod/prod/storage.tfstate"     # Key for the production storage layer state
    region = "ap-southeast-2"                     # Must match backend region
  }
}
...
# Get Default VPC -> Real prod we can opt for custom VPC, but out-of-scope for this project
